<script lang="ts">
  import type { EconomicEvent, Intent } from '$lib/schemas';
  import { computeADU, computeForwardADU } from '$lib/algorithms/ddmrp';
  ...
  let { specId, events, intents, today }: Props = $props();
      ...
      {#each Object.entries(seriesColors) as [label, color]}
        <span class="legend-item" style:color={color}>
          <svg width="16" height="4" style="vertical-align:middle;margin-right:2px">
            <line x1="0" y1="2" x2="16" y2="2" stroke={color} stroke-width="2"
              stroke-dasharray={label === '1-week' ? '3,2' : label === '12-week' ? '5,2' : 'none'} />
          </svg>
          {label}
        </span>
      {/each}
      ...
      {#each Object.keys(seriesColors) as series}
        ...
        {#if seriesData.length > 0}
          <Line
            ...
            strokeWidth={series === '52-week' ? 2 : 1.5}
            strokeDasharray={series === '1-week' ? '3,2' : series === '12-week' ? '5,2' : undefined}
          />
        {/if}
      {/each}
