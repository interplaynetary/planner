<script lang="ts">
	...
	// --- Non-reactive refs (must NOT be $state proxies) ---
	let room: any = null;
	...
	// Mirror status into shared state so AppBar can reflect it
	...
	async function tune(name: string) {
		...
		try {
			...
			room.onPeerJoin((peerId: string) => {
				peers = [...peers, peerId];
				// share mic stream with newly joined peer if already speaking
...
{#if !tuneState.expanded}
	{#if status === 'live'}
		<!-- Compact live pill -->
		<div class="widget collapsed live">
			<span class="live-dot"></span>
			<button class="topic-name" title="Copy link" onclick={copyLink}>{topic}</button>
			<span class="sep">·</span>
			<button
				class="ctrl-btn"
			...
			<span
				class="live-dot"
				style={listening ? '' : 'background: var(--zone-red); animation: none;'}
			></span>
			<span class="peer-count">{peers.length}</span>
			<button class="expand-btn" title="Expand" onclick={() => (tuneState.expanded = true)}>▼</button>
		</div>
	{/if}
{:else}
	<!-- Expanded panel -->
	<div class="widget expanded" class:live={status === 'live'}>
		<div class="panel-header">
			<span class="label">TUNE IN</span>
			<button class="close-btn" title="Collapse" onclick={() => (tuneState.expanded = false)}>×</button>
		</div>
		...
		<div class="tune-row">
			<input
				...
				onkeydown={(e) => e.key === 'Enter' && tune(inputTopic)}
			/>
			{#if status === 'live' && topic}
				<button class="icon-btn" title="Copy link" onclick={copyLink}>📋</button>
			{/if}
		</div>
		<button
			class="tune-btn"
			disabled={status === 'connecting'}
			onclick={() => tune(inputTopic)}
		>
			{status === 'connecting' ? 'Connecting…' : 'TUNE'}
		</button>
...
<style>
	...
	/* ── Expanded panel + live collapsed pill ── */
	.widget {
		font-size: 11px;
