...
<div class="piv">
  ...
  <div class="right">
    <!-- CTT Gate -->
    <div class="section-label">Customer Tolerance Time (CTT)</div>
    <div class="ctt-row">
      <label class="input-row">
        <span class="input-label">CTT</span>
        <input type="number" min="0" bind:value={customerToleranceTimeDays} class="num-input" placeholder="days" />
        <span class="input-label muted">days</span>
      </label>
      {#if customerToleranceTimeDays != null && selectedCandidate}
        {@const gap = selectedCandidate.decoupledDltDays - customerToleranceTimeDays}
        {#if gap <= 0}
          <span class="ctt-badge meet">✓ DLT meets CTT</span>
        {:else}
          <span class="ctt-badge miss">✗ {gap.toFixed(0)}d above CTT</span>
        {/if}
      {:else if customerToleranceTimeDays != null && currentOutputZone}
        {@const gap = currentOutputZone.dltDays - customerToleranceTimeDays}
        {#if gap <= 0}
          <span class="ctt-badge meet">✓ Current DLT meets CTT (no decoupling needed)</span>
        {:else}
          <span class="ctt-badge miss">✗ Current DLT {gap.toFixed(0)}d above CTT — decoupling required</span>
        {/if}
      {/if}
    </div>
    <p class="hint">
      CTT is the maximum lead time your customer will accept. Goal: compress DLT below CTT by placing buffers at strategic decoupling points (Ch. 6).
    </p>
    ...
    {:else if !selectedRecipeId}
      <div class="explainer">
        ...
        <p>This loop places decoupling point buffers inside your recipe chains to compress customer-facing lead times.</p>
        <ol>
          <li>Set your <strong>Customer Tolerance Time (CTT)</strong> — the max lead time your market accepts.</li>
          <li>Select a <strong>recipe</strong> — candidates are ranked by lead-time compression × cross-recipe leverage.</li>
          <li>Pick a candidate → review the before/after buffer impact → accept to create the buffer and re-size the parent zone.</li>
          <li>Repeat until compressed DLT ≤ CTT.</li>
        </ol>
      </div>
...
<style>
  ...
  .ctt-row { display: flex; align-items: center; gap: var(--gap-md); flex-wrap: wrap; }
  .ctt-badge { font-family: var(--font-mono); font-size: var(--text-xs); padding: 2px 8px; border-radius: 2px; }
  .ctt-badge.meet { background: rgba(72,187,120,0.15); color: var(--zone-green); border: 1px solid var(--zone-green); }
  .ctt-badge.miss { background: rgba(245,101,101,0.12); color: var(--zone-red); border: 1px solid var(--zone-red); }
  .hint { font-size: var(--text-xs); opacity: 0.45; margin: 2px 0; line-height: 1.5; }
  .explainer { display: flex; flex-direction: column; gap: var(--gap-sm); max-width: 480px; }
